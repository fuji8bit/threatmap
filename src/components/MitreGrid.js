...
const MitreGrid = (props) => {
    const [tableData, setTableData] = useState([]);
    const [pageSize, setPageSize] = useState(10);

    const query = { query: "g.V().hasLabel('Sub-Technique').elementMap()" }
    const l_width = 400;
    const m_width = 200;
    const s_width = 100;
    ...
    const columns = [
        ...
        {field: 'name', headerName: 'Name', width: m_width, editable: false},
        {field: 'link', headerName: 'Link', width: m_width, editable: false},
        {field: 'description', headerName: 'Description', width: l_width, editable: false},
    ];
